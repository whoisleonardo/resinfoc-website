  ...
  return (
    <div>
      <section className="container" style={{ padding: '36px 32px 32px' }}>
        ...
        <p style={{ fontSize: 17, lineHeight: 1.6, color: 'var(--ink)', maxWidth: 640, margin: '0 0 28px' }}>
          {section.subtitle}
        </p>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  padding: '10px 18px',
                  borderRadius: 999,
                  border: `1.5px solid ${active ? 'var(--brand-ink)' : 'color-mix(in srgb, var(--brand-ink) 30%, transparent)'}`,
      ...
      {section.bannerText && (
        <section className="container" style={{ padding: '32px 32px 24px' }}>
          <div style={{ background: 'var(--surface-muted)', border: '1.5px dashed color-mix(in srgb, var(--brand-ink) 40%, transparent)', borderRadius: 16, padding: '22px 26px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20, flexWrap: 'wrap' }}>
            <p style={{ fontSize: 15, color: 'var(--ink)', margin: 0 }}>{section.bannerText}</p>
            {section.bannerUrl && (
              <a href={section.bannerUrl} target="_blank" rel="noreferrer" style={{ flex: 'none', maxWidth: '100%', font: '700 15px var(--font-display)', color: 'var(--surface)', background: 'var(--brand-ink)', padding: '12px 22px', borderRadius: 999, textDecoration: 'none' }}>
      ...
      <section className="container" style={{ padding: '24px 32px 100px' }}>
        <div className="grid-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 26 }}>
          ...
          {filtered.map((m) => {
            const cardStyle = { textDecoration: 'none', display: 'block', width: '100%', textAlign: 'left', padding: 0, cursor: 'pointer', background: 'var(--surface)', borderRadius: 20, overflow: 'hidden', border: '1.5px solid color-mix(in srgb, var(--brand-ink) 16%, transparent)' };
            const inner = (
              <>
